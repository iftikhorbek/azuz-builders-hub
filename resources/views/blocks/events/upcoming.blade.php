                ...
                <div class="p-6 card-elevated bg-card border-0 flex flex-col rounded-xl" x-data="{ open: false }">
                    ...
                    <div class="space-y-2 mb-4 pb-4 border-t pt-4 text-sm text-muted-foreground">
                        <div class="flex items-center gap-2">
                            ...
                            {{ \Carbon\Carbon::parse($event['date'] ?? now())->format('l, F d, Y') }}
                        </div>
                        <div class="flex items-center gap-2">
                        ...
                        <div class="bg-popover max-w-lg w-full rounded-xl p-6 shadow-xl" @click.outside="open = false">
                            ...
                            <form class="grid gap-4">
                                <div class="grid gap-2">
                                    <label class="text-sm font-medium text-foreground" for="name-{{ $event['id'] ?? uniqid() }}">Full Name</label>
                                    <input id="name-{{ $event['id'] ?? uniqid() }}" type="text" placeholder="Enter your full name" class="w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                                </div>
                                <div class="grid gap-2">
                                    <label class="text-sm font-medium text-foreground" for="email-{{ $event['id'] ?? uniqid() }}">Email</label>
                                    <input id="email-{{ $event['id'] ?? uniqid() }}" type="email" placeholder="your.email@company.com" class="w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                                </div>
                                <div class="grid gap-2">
                                    <label class="text-sm font-medium text-foreground" for="org-{{ $event['id'] ?? uniqid() }}">Organization</label>
                                    <input id="org-{{ $event['id'] ?? uniqid() }}" type="text" placeholder="Your company name" class="w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                                </div>
                                <div class="grid gap-2">
                                    <label class="text-sm font-medium text-foreground" for="phone-{{ $event['id'] ?? uniqid() }}">Phone Number</label>
                                    <input id="phone-{{ $event['id'] ?? uniqid() }}" type="tel" placeholder="+998 XX XXX XX XX" class="w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                                </div>
